...
class RMPFlowController(mg.MotionPolicyController):
    def __init__(self, name: str, robot_articulation: Articulation, physics_dt: float = 1.0/60.0):
        self.rmpflow = mg.lula.motion_policies.RmpFlow(
            robot_description_path="//home/deeptree/dev_ws/isaac_sim/src/m0609_robot_descripton.yaml",
            rmpflow_config_path="/home/deeptree/dev_ws/isaac_sim/src/m0609_rmpflow_common.yaml",
            urdf_path=M0609_URDF_PATH,
            end_effector_frame_name="link_6",  # your EE link
            maximum_substep_size=0.00334,
        )
        mg.MotionPolicyController.__init__(
            self, name=name, articulation_motion_policy=mg.ArticulationMotionPolicy(
...
class DoosanPickPlaceController:
    ...
    def __init__(self, name, gripper, robot_articulation):
        ...
        super().__init__(
            ...
            gripper=gripper,
            events_dt=events_dt,
            end_effector_initial_height=0.3,  # tune for M0609 reach
        )
        ...
        if is_playing and not reset_needed:
            if not controller.is_done():
                ...
                actions = controller.forward(
                    ...
                    end_effector_offset=np.array([0, 0, 0.0]),  # tune if needed
                )
